...
import { rateFor, unpricedReason, type TokenRate } from '@/lib/agent/rates';
...
import { indexAnalysesByPost, type AnalysisIdentity } from '@/lib/board/identity';
import {
  distinctPosts,
  scanCoverage,
  MAX_POSTS_SCAN,
...
/* -------------------------------------------------------------- the rates --
 *
 * PUBLISHED_RATES used to be declared here, which put the only verified
 * per-token table in the repository inside a route module. /api/digest builds
 * its own token ceiling and could not reach it, so it returned `usd: null` and
 * said why. The table now lives in src/lib/agent/rates.ts, unchanged: the same
 * models, the same figures, the same source URLs and read dates, and the same
 * refusal to price a model nobody has verified. `TokenRate`, `rateFor()` and
 * the reason string come from there.
 */
...
/**
 * Exactly what the matching needs from `post_analyses`, and `ig_id` is the
 * whole point of the list.
 *
 * This route used to select `post_id` alone and recover the post through an
 * in-memory map built from the capped `posts` read above. That map is bounded by
 * MAX_POSTS_SCAN: at ~320 rows per snapshot the read stops mid-history on the
 * seventh scrape, and every analysis whose cited row fell out of it looked
 * unanalysed — so the post was queued again and the model was paid a second time
 * to describe it. Migration 0004 added the column; reading it is what retires
 * that boundary. `created_at` is read because it decides which of two analyses
 * of the same post is the one shown.
 *
 * One string LITERAL, like every other select list in this file: PostgREST
 * parses the column list at the type level.
 */
// prettier-ignore
const ANALYSIS_IDENTITY_COLUMNS = 'post_id, ig_id, created_at';

/**
 * The pre-0004 bridge, and nothing more. Rows written before the column existed
 * carry a null `ig_id`, and the only place theirs can be recovered from is the
 * scrape rows this request already read — which is precisely the capped lookup
 * the column replaced, so it is the fallback and never the rule. Built from the
 * PRE-collapse rows: the row a superseded analysis cites is the one the collapse
 * discarded, and that row is the only place its ig_id is.
 */
...
async function progress() {
  ...
  const [rank, postsResult, analysesResult] = await Promise.all([
    ...
    db.from('post_analyses').select(ANALYSIS_IDENTITY_COLUMNS),
  ]);
  ...
  const index = indexAnalysesByPost(
    (analysesResult.data as AnalysisIdentity[] | null) ?? [],
    population.posts,
    legacyIgIdByRowId(rows),
  );

  const total = population.posts.length;
  const analyzed = index.byIgId.size;

  return {
    total,
    analyzed,
    /** …of which the analysis hangs off the row the board now displays. */
    analyzed_current: index.analyzed_current,
    /** …and of which it was written against an earlier scrape of the same post. */
    analyzed_superseded: index.analyzed_superseded,
    remaining: total - analyzed,
    /**
     * Analyses naming a post this population does not contain — deleted, or
     * past the row cap. Stated, never folded into "unanalysed": an analysis
     * that could not be placed is not the same fact as a post that has none.
     */
    unresolved_analyses: index.unresolved,
    /**
...
export async function POST(request: Request) {
  try {
    ...
    // THE LINE THAT DECIDES RE-SPEND. By ig_id, read from the analysis row
    // itself: a post analysed before the last re-scrape has a NEW winning row
    // with a NEW uuid, so filtering on row id would queue it — and charge for it
    // — all over again. Reading the stored column rather than inferring the join
    // from `scanRows` is also what removes the MAX_POSTS_SCAN boundary: an
    // analysis stays matched to its post however far its cited row has fallen
    // out of the capped read.
    const index = indexAnalysesByPost(
      (doneResult.data as AnalysisIdentity[] | null) ?? [],
      allPosts,
      legacyIgIdByRowId(scanRows),
    );
    const pending = allPosts.filter((post) => !index.byIgId.has(post.ig_id)).slice(0, limit);
    ...
    // Keyed by the row id the prompt showed the model, carrying the ig_id that
    // row belongs to: the model answers with `post_id`, and `ig_id` is what the
    // stored analysis will be matched by for the rest of its life.
    const byId = new Map(rows.map((r) => [r.post.id, { computed: r.computed, ig_id: r.post.ig_id }]));
    const inserts = run.value.analyses.flatMap((a) => {
      const target = byId.get(a.post_id);
      ...
      return [
        {
          post_id: a.post_id,
          /**
           * The durable join, written at insert time so no later reader has to
           * infer it from a capped scan of `posts`. `post_id` stays beside it:
           * it is a real foreign key and it is the only record of WHICH SCRAPE
           * these comparatives were computed against.
           */
          ig_id: target.ig_id,
